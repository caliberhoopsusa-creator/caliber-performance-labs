...
import { Camera, Plus, X, ChevronLeft, ChevronRight, Eye, Heart, Star, Zap, Upload, Image, Video, Clock } from "lucide-react";
        ...
        >
          ...
          >
            <Avatar className="w-10 h-10 border border-accent/30">
              <AvatarImage src={(currentStory as any).playerPhoto ?? undefined} alt={(currentStory as any).playerName} className="object-cover" />
              <AvatarFallback className="text-xs font-bold bg-accent/20 text-accent">
                {getInitials((currentStory as any).playerName || '')}
              </AvatarFallback>
            </Avatar>
          </motion.div>
          <motion.div
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {/* Only show player selector for coaches who manage multiple players */}
          {players.length > 1 && (
...
export default function StoriesContent() {
  ...
  // Players can only post for themselves; coaches see all players
  const ownPlayerId = user?.playerId;
  const players = user?.role === 'coach'
    ? allPlayers
    : allPlayers.filter(p => p.id === ownPlayerId);
  const defaultPlayerId = ownPlayerId ? String(ownPlayerId) : undefined;
