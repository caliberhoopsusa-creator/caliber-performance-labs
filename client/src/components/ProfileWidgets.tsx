...
export function ProfileWidgets({ games, selectedWidgets, onWidgetsChange, isOwnProfile, position }: ProfileWidgetsProps) {
  ...
  const availableWidgets = BASKETBALL_WIDGETS;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {widgets.includes("trends") && <TrendsWidget games={sortedGames} position={position} />}
        {widgets.includes("grades") && <GradesWidget games={sortedGames} />}
        {widgets.includes("radar") && <RadarWidget games={sortedGames} position={position} />}
        {widgets.includes("averages") && <AveragesWidget games={sortedGames} position={position} />}
...
function TrendsWidget({ games, position }: { games: Game[]; position?: string }) {
      ...
      <div className="h-48">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData}>
            ...
            <Tooltip content={<PremiumTooltip />} cursor={{ stroke: 'rgba(224,36,36,0.3)' }} />
            <Line type="monotone" dataKey="points" stroke="hsl(var(--accent))" strokeWidth={2.5} dot={{ r: 4, fill: 'hsl(var(--accent))', filter: 'drop-shadow(0 0 8px rgba(224,36,36,0.6))' }} activeDot={{ r: 6, filter: 'drop-shadow(0 0 12px rgba(224,36,36,0.8))' }} isAnimationActive name="PTS" />
            ...
            <Line type="monotone" dataKey="assists" stroke="#C6D0D8" strokeWidth={2.5} dot={{ r: 4, fill: '#C6D0D8', filter: 'drop-shadow(0 0 8px rgba(198,208,216,0.6))' }} activeDot={{ r: 6, filter: 'drop-shadow(0 0 12px rgba(198,208,216,0.8))' }} isAnimationActive name="AST" />
          </LineChart>
        </ResponsiveContainer>
      </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData}>
            ...
            <Tooltip content={({ active, payload }: any) => {
              ...
              return null;
            }} cursor={{ fill: 'rgba(224,36,36,0.1)' }} />
...
function RadarWidget({ games, position }: { games: Game[]; position?: string }) {
        ...
        <ResponsiveContainer width="100%" height="100%">
          <RadarChart data={radarData}>
            ...
            <Radar
              ...
              strokeWidth={2.5}
              isAnimationActive
              filter="drop-shadow(0 0 8px rgba(224,36,36,0.3))"
            />
          </RadarChart>
        </ResponsiveContainer>
      </div>
...
function AveragesWidget({ games, position }: { games: Game[]; position?: string }) {
  ...
  return (
    <Card className="p-4 animate-fade-up" data-testid="widget-averages">
      ...
      <div className="grid grid-cols-5 gap-2 text-center">
        <div className="p-3 rounded-lg bg-accent/10 border border-accent/20 hover-elevate transition-all">
  ...
  return (
    <Card className="p-4 animate-fade-up" data-testid="widget-shooting">
      ...
      <div className="grid grid-cols-3 gap-3 text-center">
        <div className="p-4 rounded-lg bg-blue-500/15 border border-blue-500/20 hover-elevate transition-all">
          <p className="text-2xl font-bold text-accent">{shooting.fg}%</p>
          <p className="text-xs text-muted-foreground">FG%</p>
        </div>
        <div className="p-4 rounded-lg bg-green-500/15 border border-green-500/20 hover-elevate transition-all">
          <p className="text-2xl font-bold text-green-400">{shooting.three}%</p>
          <p className="text-xs text-muted-foreground">3PT%</p>
        </div>
        <div className="p-4 rounded-lg bg-accent/15 border border-accent/20 hover-elevate transition-all">
