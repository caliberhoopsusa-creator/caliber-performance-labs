  ...
  if (event.type === "checkout.session.completed") {
    ...
    if (ownerEmail && process.env.RESEND_API_KEY) {
      ...
      await sendEmail({
        ...
        html: paidNotifyHtml(lead, baseUrl, session),
...
function customerIntakeHtml(lead: { name: string }) {
  return `
    <p>Thanks for your order. Your site for <strong>${lead.name}</strong> is in the build queue.</p>
    <p>To finish it, reply to this email with:</p>
    <ol>
    ...
    <p>Once we have those, you'll get a link to the live site within 5–7 days.</p>
  `;
}
