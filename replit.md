...
- **AI Integration**: Gemini AI for video analysis and performance projections. Video analysis features include client-side validation (format, size, duration with thumbnail preview), backend retry logic with exponential backoff, edge-case prompt engineering (low quality, multiple players, partial clips), confidence-based review/edit UX with inline stat editing, "Use Stats in Game Entry" bridge that pre-fills the game form, and analysis history (saved to `video_analyses` table) with load/delete capabilities.
